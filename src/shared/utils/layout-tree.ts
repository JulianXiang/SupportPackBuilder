...
export const repairLayoutSheetOrder = (
  sheets: readonly LayoutSheet[],
  canonicalSourcePageIds: readonly string[],
  now = new Date(),
): LayoutSheet[] => {
  const rankBySourcePageId = new Map(
    canonicalSourcePageIds.map((sourcePageId, index) => [sourcePageId, index]),
  )
  ...
  const baseOrder = sheets.length > 0 ? Math.min(...sheets.map((sheet) => sheet.order)) : 0
  return sheets
    .map((sheet) => {
    ...
    .map((sheet, index) => ({ ...sheet, order: baseOrder + index }))
}
