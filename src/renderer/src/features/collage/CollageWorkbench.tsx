...
export const CollageWorkbench = (props: CollageWorkbenchProps): React.JSX.Element => {
  ...
  const repairConfiguredOrder = (): void => {
    const canonicalIds = [...props.sources]
      .sort((left, right) => left.canonicalOrder - right.canonicalOrder)
      .map((source) => source.sourcePageId)
    const repaired = repairLayoutSheetOrder(sheets, canonicalIds)
    setSheets(repaired)
    setActiveSheetIndex(0)
      ...
      {activeSheet ? (
        <div className="collage-workbench-grid">
          ...
          <aside className="collage-control-panel">
            ...
            {props.experienceMode === 'basic' && (
              <Alert
                type="info"
                showIcon
                title="当前为基础模式"
                description="已隐藏边距、裁切坐标和手动拆分等复杂参数。"
                action={
                  <Button size="small" onClick={props.onSwitchAdvanced}>
                    高级设置
                  </Button>
                }
              />
            ...
            {props.experienceMode === 'advanced' && (
              <>
                ...
                <label className="collage-control-label">
                  槽间距 {activeSheet.slotGapPoints} pt
                </label>
                <Slider
                  min={0}
                  max={24}
            ...
            {props.experienceMode === 'advanced' && activeSheet.sections.length > 1 && (
              <>
                <label className="collage-control-label">
