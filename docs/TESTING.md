...
Vitest 覆盖页码范围、A4 几何、旋转矩阵、PagePlan、逻辑页码、三级中文编号、十以上序号、空目录排除、拖拽重排、项目 schema v1→v2→v3 迁移、全局偏好回退、问题汇总与安全修复、封面字段独立性、页面预览滚动条几何和 LibreOffice 运行时解析。
...
- 输出文件重读
- 取消、超时及导出失败后的临时文件清理
- 真实示例项目生成、项目内资产、四图拼版、无错误 PagePlan、A4 导出和失败清理
- 结构化导出预检的代码、建议、目录/材料关联，以及隔离 ElectronStore 的跨实例持久化
...
- 验证首次引导关闭与帮助页重开、基础/高级模式跨重载、真实示例项目、统一警告定位、安全修复与撤销
