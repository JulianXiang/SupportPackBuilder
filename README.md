...
- 首次启动四步引导与本机生成的可编辑示例项目；示例同样经过真实导入、保存、PagePlan 和导出流程
- 全局“基础 / 高级”模式跨重启记忆；基础模式保留常用操作，高级模式提供完整技术参数
- 统一问题与警告中心汇总文件校验和 PagePlan，支持定位、可确认的安全修复及撤销
...
1. 按欢迎页四步引导点击“新建项目”，或先创建可自由编辑、保存和导出的示例项目。
...
8. 状态栏出现缺失、错误或警告时，打开统一问题中心定位处理；安全修复会先确认并可撤销。
9. 点击“导出 PDF”，先查看结构化错误、警告和预计页数，再选择输出位置。
10. 导出完成后可打开文件、所在目录或 JSON 校验报告。
