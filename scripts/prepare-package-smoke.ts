...
const pdfMaterial = createMaterial(pdfSource, {
  title: '十页 PDF 选页材料',
  sourceType: 'pdf',
  selectedPageRanges: '1,3,5-7',
})
const imageMaterial = createMaterial(imageSource, {
  title: 'PNG 证书',
  sourceType: 'image',
  selectedPageRanges: 'all',
})
    ...
    children: [
      {
        ...
        insertDividerPage: false,
        children: [],
        materials: [pdfMaterial, imageMaterial],
      },
    ],
    materials: [],
  },
...
const lastSelectedPdfPageId = `${pdfSource.id}:6`
const imagePageId = `${imageSource.id}:0`
const smokeLayout = createLayoutSheet({
  anchorSourcePageId: lastSelectedPdfPageId,
  sections: [
    acknowledgeClarity(createLayoutSection(pdfMaterial.id, [lastSelectedPdfPageId], 'two-up')),
    acknowledgeClarity(createLayoutSection(imageMaterial.id, [imagePageId], 'two-up')),
  ],
  order: 0,
  orientation: 'portrait',
  templateId: 'multi-material-sections',
  project: officeImport.project,
  crossDirectoryConfirmed: true,
  autoGenerated: false,
})
const smokeProject = ProjectSchema.parse({
  ...officeImport.project,
  layoutSheets: [smokeLayout],
})
await writeProjectAtomically(projectDirectory, smokeProject)
...
process.stdout.write(
  `${JSON.stringify({
    ...
    collageSheetCount: smokeProject.layoutSheets.length,
  })}\n`,
)
